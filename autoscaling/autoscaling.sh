...
#create_listener
create_listener(){
  listeners_arn=$(aws elbv2 create-listener --load-balancer-arn $LB_ARN --protocol TCP --port 80 --default-actions Type=forward,TargetGroupArn="$TG_ARN" | grep -oP '(?<="ListenerArn": ")[^"]*') 
